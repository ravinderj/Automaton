...
var trim_undefined = function(item) {
  return item || ''
};
...
var runTests = function(test, methodNames, option) {
  ...
  var executeTest = function(name) {
    ...
    try {
      ...
      if (option === 'stop') throw {
        name: 'User Requested to stop',
...
var main = function() {
  ...
  report = (option === '-only') && runTests(testDetails.test, testDetails.methodNames.filter(matching));
  report = option || runTests(testDetails.test, testDetails.methodNames);
  fs.appendFileSync("./report.txt",report,"utf8");
};
